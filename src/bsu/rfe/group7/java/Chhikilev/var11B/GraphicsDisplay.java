...
public class GraphicsDisplay extends JPanel {
    ...
    private boolean showAXtremum = true;
...
// Перо для рисования графика
        graphicsStroke = new BasicStroke(2.0f, BasicStroke.CAP_BUTT,
                BasicStroke.JOIN_ROUND, 10.0f, new float[] {40,10,20,10,10,10,20,10,40,10}, 0.0f);
...
// Изменение любого параметра приводит к перерисовке области
    ...
    public  void setShowAXtremum(boolean showAXtremum){
        this.showAXtremum = showAXtremum;
        repaint();
    }
...
// Шаг 9 - Восстановить старые настройки холста
        if (showAXtremum) paintAXtremum(canvas);
...
// Отобразить график
    ...
    // локальный экстремум
    protected  void paintAXtremum(Graphics2D canvas){
        ...
        for (int i = 0; i < graphicsData.length; i++) {
            ...
            double Y = Double.valueOf(center.y);
            double X = Double.valueOf(center.x);
            Rectangle2D bounds = axisFont.getStringBounds("(X;Y)", context);
            if(i != 0){
                if(f == false){
                    if( Y1 > Y){
                        System.out.println((Y1));
                        System.out.println((Y));
                        canvas.drawString("("+X+";"+Y+")",(float) X,(float) Y);
                        f = true;
                    }
                }else{
                    if(Y1 < Y){
           ...
           // Point2D.Double labelPos = xyToPoint(c, point[1]);
...
// Центр - в точке (x,y)
            ...
            Marker.lineTo(center.x, center.y+10);
            Marker.lineTo(center.x + 5, center.y + 5);
            Marker.closePath();
